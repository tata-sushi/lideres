...
for (const id of ['integration_core','readonly_official_history','pdfjs_security','ux_information_architecture','smart_substitutions_trocas_v2','temporary_harnesses']) {
...
assert.ok(byId.readonly_official_history.arquivosMistos.includes(misto.arquivo));
assert.ok(byId.smart_substitutions_trocas_v2.arquivosMistos.includes(misto.arquivo));
...
assert.ok(mapa.procedimentoFuturo.some((x) => /patch-level/i.test(x)));
