-- ═══════════════════════════════════════════════════════════════════════════
-- Clima — chave de PAUSA global no emit (além do limitador de 20/dia)
-- ---------------------------------------------------------------------------
-- av_clima_emitir passa a checar gatilho->>'pausado' do modelo 'clima':
-- quando pausado=true, NADA é emitido (nem reaproveitado) — retorna
-- {ok:false, motivo:'pausado'}. Útil em janelas de manutenção (ex.: troca do
-- número/instância do WhatsApp). O valor pausado (true/false) é estado
-- operacional (ligado/desligado ao vivo), não vem fixado nesta migration.
-- Mantém o limitador diário (máx. 20 links novos/dia).
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── PAUSA GLOBAL: nada é emitido enquanto pausado (ex.: troca de número) ──
  if v_pausado then
