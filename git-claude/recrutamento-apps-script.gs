...
function avaliarTeste(e) {
  ...
  try {
    ...
    var vagaVinculada  = (e.parameter.vaga_vinculada || '').trim();
...
function atualizarStatusDevolutivaEntrevistas() {
  ...
  const ultimaLinha = aba.getLastRow();
  if (ultimaLinha < linhaInicial) { Logger.log("Sem linhas para processar."); return; }

  const quantidadeLinhas = ultimaLinha - linhaInicial + 1;
  const datas    = aba.getRange(linhaInicial, 9,  quantidadeLinhas, 1).getValues();
  const horarios = aba.getRange(linhaInicial, 10, quantidadeLinhas, 1).getValues();
  const status   = aba.getRange(linhaInicial, 12, quantidadeLinhas, 1).getValues();
  ...
  for (let i = 0; i < quantidadeLinhas; i++) {
    ...
    if (isNaN(dataHoraEntrevista.getTime())) continue;

    if (valorHorario instanceof Date && !isNaN(valorHorario.getTime())) {
      dataHoraEntrevista.setHours(valorHorario.getHours(), valorHorario.getMinutes(), valorHorario.getSeconds(), 0);
    } else {
