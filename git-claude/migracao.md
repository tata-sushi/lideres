...
### Sistema de Avaliações & Feedback (pesquisas por WhatsApp) — Clima ✅ no ar · Desligamento ⏸️ em espera
> Instrumentos data-driven em `dp_rh.avaliacao_modelos` (slug + `form` jsonb + `gatilho` jsonb). Disparo por n8n → RPC `av_*_emitir` (mina token, respeita pausa/intervalo/limitador/reaproveitamento) → WhatsApp (uazapi). Página pública no repo `tata-sushi/pesquisa`. Regra de linguagem fixa: **sempre "confidencial", nunca "anônimo"** (o backend rastreia identidade; resultados são por grupo).
- [x] **Clima — NO AR desde 14/09.** Config final: **7 pessoas/dia** (limitador no banco = 7 **e** amostra do n8n = 7), **5 perguntas/disparo**, **intervalo 30 dias/pessoa**, reaproveita link ativo, TTL 24h. Workflow `l8tWvmZIm8mH02lH` (**ativo/publicado**, disparo diário **10h**). Trilho de segurança = `gatilho.pausado` (pausa removida no go-live). Dashboard: aba **Cultura & Clima** (`compliance/kpis/rh/cei.html`) — radar Geral × Liderança, filtros unidade/departamento, comentários. Base zerada no lançamento (0 respostas). Workflow de teste isolado `sGF2PlcCcMDWWPgD` mantido (+ 1 token de teste na base) — aposentar quando não for mais preciso.
- [ ] **Desligamento (entrevista de saída) — EM ESPERA** (usuário: "ainda vamos esperar", 14/09). **Já feito:** instrumento (modelo + tabelas + RPCs), **93 respostas históricas migradas**, página `desligamento.html` no repo `pesquisa`. **Falta p/ ir ao ar:** o **disparo automatizado** — workflow n8n `sbO8dZPTmsNanAj6` (hoje **inativo**): revisar a **copy** (garantir "confidencial", nunca "anônima" + typos), validar **token uazapi**, alinhar amostra/limitador/gatilho ao padrão do Clima e **ativar/publicar**. Seguir exatamente o fluxo que o Clima usou. **Não executar sem "ok" explícito do usuário.**

### Agente Observador de Grupos WhatsApp → atividades estruturadas 💡 IDEIA (21/09)
- [ ] **Agente de memória operacional** (não é só bot que salva msg). Ideia do usuário: um agente com **número próprio dentro do grupo**, em **modo exclusivamente observador**, que captura tudo, interpreta com IA e vira histórico/métricas consultáveis. **Não iniciar sem "ok" explícito.**
  - **Fluxo:** WhatsApp (grupo) → captura bruta → interpretação por IA → banco estruturado → histórico/métricas → consultas e dashboard.
  - **Pipeline detalhado:** Grupo → captura da mensagem → identifica (grupo, colaborador, data/hora, mensagem, resposta/referência, áudio/anexo) → **salva mensagem ORIGINAL** → IA interpreta → transforma em **atividades estruturadas** → Supabase/Postgres → dashboard + consultas com IA.
  - **Regra de ouro:** guardar **sempre a mensagem original**; a interpretação da IA fica em **tabela separada** — nunca ficar refém do modelo e poder **reprocessar o histórico** no futuro.
  - **Governança:** filtro de **grupos autorizados** (só processa allowlist); modo observador (não responde no grupo).
  - **A definir:** infra de captura (uazapi/Evolution/Baileys?), tabelas (`msg_raw` + `atividades`), custo/limite da IA, transcrição de áudio, e como consultar (dashboard no portal).
  - **Complexidade: média a média-alta.** Banco e dashboard são tranquilos; o peso está na **captura inbound** (receber msgs de grupo, não enviar) e na **interpretação por IA**. Blocos: captura inbound (webhook grupo + mídia) = **alta/risco, gargalo**; `msg_raw` = baixa; IA→`atividades` = média (prompt/custo/erro/reprocesso); áudio→transcrição = média; allowlist de grupos = baixa; dashboard/consultas = média.
  - **Tempo estimado:** MVP (só texto, 1 grupo piloto: captura→bruto→IA→atividades→dashboard simples) **~1–2 dias** *se* a captura inbound do gateway já funcionar; completo (áudio, vários grupos, métricas, consulta com IA) **~1 semana**.
  - **Riscos/dependências:** (1) o gateway conseguir **receber** msgs de grupo + mídia; (2) **custo da IA** por mensagem. **1º passo quando liberar:** checar no n8n/uazapi se a captura inbound de grupo é possível — destrava o resto.

### Recrutamento — modal Analítico: exibir feedback + data do próximo teste 📌 A FAZER (25/09)
- [ ] **Mostrar no modal de analítico (aba "Testes") duas infos que hoje ficam escondidas do analista:** (1) o **feedback** que o líder dá na devolutiva de teste e (2) a **data do próximo teste** quando o status é "Agendar outro teste". Hoje a aba Testes (`_aeInitTestes`/`_aeTestePanelHtml` em `compliance/kpis/rh/recrutamento.html`) só mostra Data/Entrada/Saída/Responsável/Valor/Status/Recibo por dia. Caso real citado pelo usuário: **Luiz Otavio Cezario de Sousa** (Poke — Pinheiros), teste 23/09, Cíntia deu devolutiva "Agendar outro teste", feedback "Ágil, interessado, educado e pró ativo", próximo teste 24/09 — nada disso aparece pro analista.
  - **SEM mudança de banco.** Os dados já vêm no RPC `rec_candidaturas_sandbox_listar` (usa `to_jsonb`): feedback = `dias[].observacoes` (+ `devolutiva_em`/`devolutiva_por`); data próximo teste = `data_proximo_teste` (nível candidatura). É só exibir no front.
  - **Plano proposto (aguardando validação do usuário):** (1) destaque no topo da aba Testes "🔁 Próximo teste agendado: DD/MM/AAAA" quando `data_proximo_teste` preenchida; (2) em cada Dia com observação, bloco "Feedback do líder (data)" com o texto. **Perguntas em aberto:** feedback e data devem ser **só leitura** (sugestão) ou editáveis? Mostrar feedback em **todos** os dias com observação (sugestão) ou só no último?
