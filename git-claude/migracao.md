...
- [~] **`pendencias_lideres_consolidado`** (id `ZJp796m0WaFjOKi0`): antes era placeholder (5 nós "Filtrar" literais, Consolidador desconectado do envio). Refeito: Schedule 17h → **HTTP `public.pendencias_lideres()`** → Code "Consolidador" → **1 envio uazapi por grupo WhatsApp**. Fonte única = RPC `public.pendencias_lideres()` (anon), retorna `(unidade, departamento, fonte, qtd)` agrupado em 5 fontes: `ausencias` (sem devolutiva), `exames` (realiza_exame + próxima ≤ hoje+30d), `entrevistas` ("Aguardando devolutiva"), `testes` ("Aguardando aprovação"), `experiencias` (admitido ≥2026-04-01, avaliado, não efetivado). Agrupamento **só por grupo/unidade → departamento** (sem líder, conforme decisão do usuário). Mapa unidade→grupo: Itaim/Pinheiros/Poke-Pinheiros/Administrativo; **Tatá House cai no grupo Administrativo (fallback — confirmar destino com o usuário)**. **Draft validado e salvo — falta PUBLICAR.** Preview (17/08): ausencias 678 (todas de abr–ago/2026), exames 20, entrevistas 17, testes 4, experiencias 3.
