...
| Valores monetários | sempre `R$ 1.234,56` — `fmtMoney` retorna `'R$ ' + n.toLocaleString('pt-BR', {minimumFractionDigits:2,maximumFractionDigits:2})` |

---

## Modal-row (tabela de dados dentro do modal)

Usar para exibir campos label/valor dentro do conteúdo do modal (ex.: modal de remuneração, ficha de colaborador).
...
Helper JS para gerar linhas dinamicamente:

```javascript
function remRow(label, val) {
...
## Sub-modal (modal sobre modal)

Quando um sub-modal (bottom sheet) abre sobre o modal principal no mobile, o modal principal deve ser ocultado para evitar sobreposição de dois bottom sheets.

```css
/* Oculta o overlay principal enquanto sub-modal está aberto */
.overlay.overlay--hidden { display: none; }
```

```javascript
function openSubModal(subId) {
  // Oculta o modal principal
  document.getElementById('overlay').classList.add('overlay--hidden');
  document.getElementById(subId).classList.add('open');
}

function closeSubModal(subId) {
  document.getElementById(subId).classList.remove('open');
  // Restaura o modal principal
  document.getElementById('overlay').classList.remove('overlay--hidden');
}
```

**Regra:** sub-modal fecha APENAS pelo seu próprio botão ✕ — igual ao modal principal.
